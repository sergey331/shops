<section id="limited-offer" class="py-28 bg-counter bg-cover bg-no-repeat bg-center">
    <div class="container mx-auto px-4">
        <!-- Swiper -->
        <div class="swiper limited-offer-swiper">
                ...
                <div class="swiper-slide">
                    <div class="flex flex-col md:flex-row items-center h-full">
                        ...
                        <div class="w-full md:w-5/12 md:ml-8 mt-10 md:mt-0 text-center md:text-left">
                            <h2 class="text-4xl md:text-5xl font-bold mb-6">{{ $discount->name }}</h2>
                            <p class="font-bold mb-6">{{ $discount->description ?? '' }}</p>
                            <p class="font-bold mb-6">{{ showDiscount($discount, setting()->currency->symbol ?? '', 'value') }}</p>

                            <!-- Countdown Timer -->
                            <div
                                    id="countdown-clock-{{ $discount->id }}"
                                    data-date="{{ $discount->finished_at ?? '' }}"
                                    data-days="{{ $discount->finished_at ? '' : setting()->default_discount_days }}"
                                    class="text-dark flex items-center my-8 countdown-clock"
                            >
